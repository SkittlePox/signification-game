...
python3 -u ippo_ff.py WANDB_NOTES="Post-Draft-Part1a-R1 - 1930 but asymmetric - Zero listener rewards" ENV_KWARGS.channel_ratio_fn=sigmoid-custom ENV_KWARGS.sigmoid_offset=498 ENV_KWARGS.sigmoid_stretch=0.007 ENV_KWARGS.sigmoid_height=0.6 ENV_KWARGS.speaker_assignment_method="arange" ENV_KWARGS.speaker_whitesum_penalty_coef=0.0 SPEAKER_TRAIN_SCHEDULE="off then on at 400" LISTENER_LR_SCHEDULE="1e-4 jump to 1e-6 at 400 anneal to 2e-5 at 1500" SPEAKER_LR_SCHEDULE=2e-4 L2_REG_COEF_LISTENER=1e-5 L2_REG_COEF_SPEAKER=1e-5 WANDB_MODE="online"
