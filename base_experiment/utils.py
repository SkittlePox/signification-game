...
def get_speaker_action_transform(fn_name, image_dim):
    ...
    def paint_multiple_splines_with_intensity_by_phone(all_spline_params: jnp.array):
        ...
        def paint_spline_on_canvas(spline_params: jnp.array):
            ...
            # P0, P1, P2 = spline_params.reshape((3, 2)) 
            P0, P1, P2, W = spline_params[0:2], spline_params[2:4], spline_params[4:6], spline_params[6]
            W *= 0.003  # This is the weight param. -0.005 is too dark. -0.002 may be too light.    # NOTE: This is negative in real rendering, positive for per-phone rendering
            t_values = jnp.linspace(0, 1, num=50)
            spline_points = bezier_spline(t_values, P0, P1, P2)
            x_points, y_points = jnp.round(spline_points).astype(int).T
        ...
        # Vmap over splines and sum contributions
        all_spline_params = jnp.clip(all_spline_params, 0.0, 1.0)
        canvases = jnp.clip(paint_spline_on_canvas(all_spline_params.reshape(-1, 7)), 0.0, 1.0)
    ...
    def paint_multiple_splines_relative(all_spline_params: jnp.array):
        ...
        @jax.vmap
        def paint_spline_on_canvas_relative(spline_params: jnp.array):
            """Paint a single spline on the canvas with specified thickness using advanced indexing."""
            ...
            # P0, P1, P2 = spline_params.reshape((3, 2)) 
            P0, P1, P2 = spline_params[0:2], spline_params[0:2]+spline_params[2:4]-0.5, spline_params[0:2]+spline_params[4:6]-0.5
            t_values = jnp.linspace(0, 1, num=50)
            spline_points = bezier_spline(t_values, P0, P1, P2)
            x_points, y_points = jnp.round(spline_points).astype(int).T
        ...
        # Vmap over splines and sum contributions
        all_spline_params = jnp.clip(all_spline_params, 0.0, 1.0)
        canvas = jnp.clip(paint_spline_on_canvas_relative(all_spline_params.reshape(-1, 6)).sum(axis=0), 0.0, 1.0)
        return canvas
    
    @jax.vmap
    def paint_multiple_splines_relative_with_intensity(all_spline_params: jnp.array):
        """Paint multiple splines on a single canvas. Requires speaker_action_dim be a multiple of 7."""
        ...
        def paint_spline_on_canvas_relative(spline_params: jnp.array):
            ...
            # P0, P1, P2 = spline_params.reshape((3, 2)) 
            P0, P1, P2, W = spline_params[0:2], spline_params[0:2]+spline_params[2:4]-0.5, spline_params[0:2]+spline_params[4:6]-0.5, spline_params[6] # +
            # P0, P1, P2, W = spline_params[0:2]+spline_params[2:4]-0.5, spline_params[2:4], spline_params[4:6]+spline_params[2:4]-0.5, spline_params[6] # ++
            W *= -0.003  # This is the weight param. -0.005 is too dark. -0.002 may be too light.
            t_values = jnp.linspace(0, 1, num=50)
            spline_points = bezier_spline(t_values, P0, P1, P2)
            x_points, y_points = jnp.round(spline_points).astype(int).T
        ...
        # Vmap over splines and sum contributions
        all_spline_params = jnp.clip(all_spline_params, 0.0, 1.0)
        canvas = jnp.clip(paint_spline_on_canvas_relative(all_spline_params.reshape(-1, 7)).sum(axis=0) + background_shade, 0.0, 1.0)
        return canvas
    
    @jax.vmap
    def paint_multiple_splines_with_intensity_bucketed(all_spline_params: jnp.array):
        """Paint multiple splines on a single canvas but quantize them. Requires speaker_action_dim be a multiple of 7."""
        ...
        # Quantize into buckets here.
        num_buckets = 6
        all_spline_params = jnp.round(all_spline_params * num_buckets) / num_buckets
    ...
    if fn_name == "identity":
        ...
        return paint_multiple_splines_relative_with_intensity
    elif fn_name == "splines_weight_bucketed":
        return paint_multiple_splines_with_intensity_bucketed
